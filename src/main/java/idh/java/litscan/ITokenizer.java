...
/**
 * The {@code ITokenizer} interface defines the contract for components that
 * split a given {@link String} into a sequence of {@link Token} objects.
 * 
 * <p>
 * Tokenizers implementing this interface are responsible for analyzing the
 * input text and identifying valid tokens according to clearly defined boundary
 * rules. These rules are tailored to preserve both token content and their
 * precise positions within the original string, which is essential for later
 * tasks like indexing, concordance generation, or highlighting search results.
 * </p>
 * 
 * <p>
 ...
 * </p>
 * <ul>
 * <li>Skip all non-printable characters and whitespace when forming
 * tokens.</li>
 * <li>Accurately compute the character offsets (begin and end) for each token
 * in the source text.</li>
 * <li>Preserve whitespace-separated structure (i.e., multiple spaces are not
 * collapsed).</li>
 * <li>Return the tokens in their original textual order.</li>
 * </ul>
 * 
 * <p>
 * This interface is central to the preprocessing pipeline of the LitScan
 * system, serving as the first step in corpus ingestion and analysis.
 * </p>
 * 
 * @see Token
 * 
